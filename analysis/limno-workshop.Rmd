...
* **R & R Studio on your computer (recommended)**
  * R can be installed by:
      ...
      + Select the `Download R for` link that is appropriate for your computer  
      + Download the latest release by clicking the corresponding 
      + Double click on the downloaded file (check `Downloads`). Follow the prompts to install.
      + If you are using macOS, you'll also need to install XQuartz [**here**](https://www.xquartz.org/).
    
  * RStudio can be installed by: 
      + Go to [**RStudio**](https://rstudio.com/products/rstudio/download/)
      + Click the `Download` button under the free, open-source license of **RStudio Desktop**
      + Download the file that is appropriate for your computer, open the downloaded file, and follow the prompts. 
...
log10(4) # common log
```
Note that we used functions here. R as a bunch of built-in functions for you to use. We can also define our own. If you want to see what a function does, simply put a question mark in front of it. Any information about the function will show up in the bottom right under the **Help** tab.
```{r intro-help}
?log()
```
...
Essentially everything we create in R is an object. Objects can be anything from numbers to figures to dataframes. To create an object, you can use the `<-` operator:
```{r intro-object}
x <- 3 * 4
```

Note that if you run this code chunk, nothing shows up. However, if you look under the **Environment** tab (top right) there should be a value for the object `x`. To view the object, we can simply call the object name directly:
```{r intro-object-call}
x
```
...
Most of the data you will come across in the environmental field will be data stored in things like Excel spreadsheets and comma-separated values files (ie. csv). R allows you to import a variety of file types with ease. You may also run into times where it's just easier to create a dataframe directly in R. We'll cover how to do both. 
...
If you want to import Excel files, you'll need to install the pacakge `readxl`. Remember that packages are installed with `install.packages()` and to only install a package once (hint: do it directly on the command line)
...
my_dataframe <- readxl::read_excel(here("folder/with/the/data", "my_awesome_excel_data.xlsx"))
```
